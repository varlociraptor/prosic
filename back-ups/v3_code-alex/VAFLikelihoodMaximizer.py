...
class VAFLikelihoodMaximizer: 
	...
	def loglikelihood (self, vaf):
		...
		for a in self.paired_end_alignments: # internal segment based evidence
			logl = logl + ln(a.probability * (vaf * self.f(a.value, self.mu + self.delta, self.sigma) + (1 - vaf) * self.f(a.value, self.mu, self.sigma)) + (1.0 - a.probability)) #TODO
		for a in self.overlapping_alignments: # overlapping read evidence
			p = a.probability 
			if a.value == 1: 
				logl = logl + ln(a.probability * (vaf * (1.0 - self.epsilon1) + (1.0 - vaf) * self.epsilon0) + (1.0 - a.probability)) #TODO
			else: 
				logl = logl + ln(a.probability * (vaf * self.epsilon1 + (1.0 - vaf) * (1.0 - self.epsilon0)) + (1.0 - a.probability)) #TODO
		return logl 
