...
class Indel: 
	...
	def __init__(self, vcf_record):
		...
		self.chromosome = 'chr' + str(vcf_record.CHROM)
...
class Deletion(Indel):
	...
	def __init__(self, vcf_record):
		...
		if self.length % 2 == 0: # when lenght is even, there are two 'centerpoints'
...
class PairedEndAlignment:
	...
	def determineAlignmentProbability(self): 
		"""
			Returns alignment probability of paired-end read. Assumes independence between read ends. The probability is capped at 95%, since we believe no alignment should be trusted too much.
		"""
...
class OverlappingAlignment: 
	...
	def determineAlignmentProbability(self):
		"""
			Determines alignment probability of this overlapping read. Note that the probability is capped at 95%, since we believe that no alignment should be trusted too much.
		"""
...
class BAMProcessor: 
	...
	def fetchAlignments (self, chromosome, start, end): 
		...
		for alignment in self.bam_reader.fetch(chromosome, max(0, start - self.search_range), end + self.search_range):
			if not alignment.is_unmapped and not alignment.is_secondary: # alignment is primary (secondary alignments are ignored) and mapped
