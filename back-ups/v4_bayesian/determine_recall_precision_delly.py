...
Determines the recall and precision (as defined in the MATE-CLEVER paper) for the deletions in the raw Delly VCF. 

	<delly-vcf> 	The Delly VCF file as provided by Tobias.
"""
...
class DellyDeletion(DDeletion):
	...
	def isSomatic(self):
		...
		for call in self.vcf_record.samples:
			if call.sample == 'hiseq.Control.wholegenome.bwamemm.cat.sorted':
				if call.gt_nums != None: 
					control = strip(call.gt_nums)
			if call.sample == 'hiseq.Cancer40.wholegenome.bwamemm.cat.sorted':
				if call.gt_nums != None: 				
...
def returnLength(delly_vcf_record):
	return int(delly_vcf_record.INFO['SVLEN'])
...
def main():
	...
	list_of_autosomes = range(1,2) # TODO TODO change back
	...
	print('Reading in the Delly VCF...')
	...
	print('Done reading in the Delly VCF...')
