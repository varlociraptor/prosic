...
class OverlappingAlignment: 
	...
	def determineSupportInsertion(self, insertion):
		...
		i = self.start
		while i <= self.end:
			for (cigar_type, cigar_length) in self.alignment.cigar:
				if (cigar_type == 1): # insertion
					self.splits.append([i, i + cigar_length - 1])
				i += cigar_length

		# walk through all splits and see whether there is one that supports the presence of the insertion
		for split in self.splits: 
			length_split = split[1] - split[0] + 1
			#if abs(length_split - insertion.length) > 20:
			#	continue
			self.value = 1
			#centerpoints_split = None 
			#if length_split % 2 == 0: # when length is even, there are two centerpoints
