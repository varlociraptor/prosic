...
void likelihood (mpf_t* l_final, double h_vaf, double c_vaf, data D) {
	...
	for (i = 0; i < D.h_isize_n; i ++) {
		mpf_set_d(obs_l, D.h_isize_prob[i] * (h_vaf*f(D.h_isize[i], MU_H + D.delta, SIGMA_H) + (1.0 - h_vaf)*f(D.h_isize[i], MU_H, SIGMA_H)) + (1.0 - D.h_isize_prob[i])) ; 
		mpf_mul(l, l, obs_l) ; 
	}
	...
	for (i = 0; i < D.c_isize_n; i ++) {
		mpf_set_d(obs_l, D.c_isize_prob[i] * (ALPHA * (h_vaf*f(D.c_isize[i], MU_H + D.delta, SIGMA_H) + (1.0 - h_vaf)*f(D.c_isize[i], MU_H, SIGMA_H)) + (1.0 - ALPHA) * (c_vaf*f(D.c_isize[i], MU_C + D.delta, SIGMA_C) + (1.0 - c_vaf)*f(D.c_isize[i], MU_C, SIGMA_C))) + (1.0 - D.c_isize_prob[i])) ; 
		mpf_mul(l, l, obs_l) ; 
	}
...
data removeUnlikelyInsertSizes (data D) {
	...
	size_t i ; 
	for (i = 0 ; i < D.h_isize_n ; i ++) {
		if (f(D.h_isize[i], MU_H + D.delta, SIGMA_H) != 0.0 || f(D.h_isize[i], MU_H, SIGMA_H) != 0.0) {
	...
	for (i = 0 ; i < D.c_isize_n ; i ++) {
		if (f(D.c_isize[i], MU_C + D.delta, SIGMA_C) != 0.0 || f(D.c_isize[i], MU_C, SIGMA_C) != 0.0) {
...
size_t uniqueGlobalMaximumExists (data D) {
	...
	for (i = 0; i < D.c_isize_n; i ++) {
		if (D.c_isize_prob[i] != 0.0) {
			if (f(D.c_isize[i], MU_C, SIGMA_C) != f(D.c_isize[i], MU_C + D.delta, SIGMA_C)) {
				c_vaf_max_exists = 1 ; 
				break ; 
			}
		}
	...
	for (i = 0; i < D.h_isize_n; i ++) {
		if (D.h_isize_prob[i] != 0.0) {
			if (f(D.h_isize[i], MU_H, SIGMA_H) != f(D.h_isize[i], MU_H + D.delta, SIGMA_H)) {
				return 1 ; 
			}
		}
	}
