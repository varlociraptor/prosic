...
void usage(const char *pname) {
	printf(	"Usage: %s [OPTION] <filename.observations>\n"
		...
		" -e\tNUM\tProbability of a split presence when the indel is not there.\n"
		" -E\tNUM\tProbability of a splits absence when the indel is there.\n"
...
void parse_arguments(parameters* p, char* input_filename, int argc, char **argv) {
	...
	size_t ch ; 
	while ((ch = getopt(argc, argv, "a:e:E:m:s:f:F:g:G:l:P:M:N:BdiIuvh")) != -1)
    	{
       		switch(ch) {
			case 'a': p->alpha = strtod(optarg, 0); break ; 
			case 'e': p->eps_a = strtod(optarg, 0); break ; 
			case 'E': p->eps_p = strtod(optarg, 0); break ; 
	...
	if (p->eps_a < 0.0 || p->eps_a >= 1.0) {
		printf("ERROR: Invalid argument. epsilon_a (-e) must lie in [0,1).\n") ;
		error_occurred = 1 ;   
	}

	if (p->eps_p < 0.0 || p->eps_p >= 1.0) {
		printf("ERROR: Invalid argument. epsilon_p (-E) must lie in [0,1).\n") ;
		error_occurred = 1 ;  
	}
...
size_t obtainVariant(FILE *fp, variant *v, parameters *p) {
	 ...
	 */
	char ch ; 
	if(fscanf(fp, "%c%zd%zd%zd%c", &(v->type), &(v->autosome), &(v->position), &(v->length), &ch) != 5){
