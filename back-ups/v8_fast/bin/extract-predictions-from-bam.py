...
def main():
	...
	parser.add_option("-l", action="store", dest="min_length", default=0, type=int,
				  help="Minimal size of events to be extracted (default=0, i.e., all).")
		...
		for event, length in read_aln.cigar:
			if event in [0,7,8]: # Match/mismatch
				...
				if length >= options.min_length:
					insertions[(read_aln.tid, ref_pos, length, read_aln.query[query_pos:query_pos+length])] += 1
					is_split_read = True
	...
	l = [(chr_id, start, length, count, sequence) for (chr_id, start, length, sequence), count in insertions.iteritems()]
	l.sort()
	for chr_id, start, length, count, sequence in l:
		print(inputfile.getrname(chr_id), start, length, 'INS', sequence, count)
