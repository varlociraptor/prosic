...
def main():
	...
	parser.add_option("-c", action="store", dest="centerpoints_distance_thres_del", default=50, type=int,
				  		help="Maximal difference in centerpoints between split and deletion for the read to be considered evidence for the deletion. (Default = 50bp)")
	parser.add_option("-C", action="store", dest="centerpoints_distance_thres_ins", default=50, type=int,
				  		help="Maximal difference in centerpoints between split and insertion for the read to be considered evidence for the insertion. (Default = 50bp)")
	parser.add_option("-l", action="store", dest="length_thres_del", default=20, type=int,
				  		help="Maximal difference in length between split and deletion for the read to be considered evidence for the deletion. (Default = 20bp)")
	parser.add_option("-L", action="store", dest="length_thres_ins", default=20, type=int,
				  		help="Maximal difference in length between split and insertion for the read to be considered evidence for the insertion. (Default = 20bp)")
	...
	bam_healthy_processor 		= BAMProcessor(bam_healthy_filename, search_range = options.search_range, primary_alignments_only = options.primary_only, centerpoints_thres_del = options.centerpoints_distance_thres_del, centerpoints_thres_ins = options.centerpoints_distance_thres_ins, length_thres_del = options.length_thres_del, length_thres_ins = options.length_thres_ins) # processes the BAM file of the control/healthy sample
 	bam_cancer_processor 		= BAMProcessor(bam_cancer_filename, search_range = options.search_range, primary_alignments_only = options.primary_only, centerpoints_thres_del = options.centerpoints_distance_thres_del, centerpoints_thres_ins = options.centerpoints_distance_thres_ins, length_thres_del = options.length_thres_del, length_thres_ins = options.length_thres_ins) # processes the BAM file of the cancer sample
