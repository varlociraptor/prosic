...
def main():
	...
	#parser.add_option("-c", action="store", dest="certainty_threshold", default=0.0, type=float, 
	#					help="A call is made only when the largest posterior probability for a value of the healthy VAF (0.0/0.5/1.0) is above or equal to this threshold. (Default = 0.0)")
	...
	for vcf_record in vcf_reader:  
		...
		if options.ploidy is None: # cancer VAF is continuous on the unit interval 
			...
			if sm_likelihood_maximizer.globalMaximumExists():
				...
				p_somatic_mutation, p_germline, p_not_present = sm_likelihood_maximizer.determinePosteriorProbabilitiesHypotheses(prior = [1/float(3.0), 1/float(3.0), 1/float(3.0)])
				...
				#p_value = sm_likelihood_maximizer.returnPValue(c_vaf_mle0)
