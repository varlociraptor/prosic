...
usage = """%prog [options] <vcf-file> <bam-healthy> <bam-cancer> <output-vcf>
...
def main():
	...
	parser.add_option("-l", action="store", dest="min_length", default=10, type=int,
				  		help="Minimal length of an indel to be considered. (Default = 10 bp)")
	parser.add_option("-m", action="store", dest="mean_insert_size_distr_healthy", default=None, type=float,
				  		help="Mean of the insert size distribution for the healthy sample. (Is estimated by default.)")
	parser.add_option("-M", action="store", dest="mean_insert_size_distr_cancer", default=None, type=float,
				  		help="Mean of the insert size distribution for the disease/cancer sample. (Is estimated by default.)")
	...
	parser.add_option("-s", action="store", dest="std_insert_size_distr_healthy", default=None, type=float,
				  		help="Standard deviation of the insert size distribution for the healthy sample. (Is estimated by default.)")
	parser.add_option("-S", action="store", dest="std_insert_size_distr_cancer", default=None, type=float,
				  		help="Standard deviation of the insert size distribution for the disease/cancer sample. (Is estimated by default.)")
	...
	annotated_vcf_filename 	= os.path.abspath(args[3])
	...
	# TODO determine mu and sigma for both the BAM files and pass them on to the LikelihoodMaximizer

 	sm_likelihood_maximizer 	= SMLikelihoodMaximizer(112, 15, 112, 15, alpha = options.alpha, epsilon_a = options.epsilon_a, epsilon_p = options.epsilon_p, cancer_vaf_range = None, tolerance=options.tolerance) # deals with the likelihood function # TODO change 

	# allocate memory
	h_paired_end_alignments, h_overlapping_alignments, c_paired_end_alignments, c_overlapping_alignments = None, None, None, None
	...
	for vcf_record in vcf_reader:  
		...
		if is_deletion:
			...
			h_isize, h_isize_prob, h_splits, h_splits_prob = bam_healthy_processor.processDeletion(deletion)
			c_isize, c_isize_prob, c_splits, c_splits_prob = bam_cancer_processor.processDeletion(deletion)
		else:
			insertion = Insertion(vcf_record)
			# Obtain the evidence (internal segment based and overlapping alignments): 
			h_isize, h_isize_prob, h_splits, h_splits_prob = bam_healthy_processor.processInsertion(insertion)
			c_isize, c_isize_prob, c_splits, c_splits_prob = bam_cancer_processor.processInsertion(insertion)

		if options.align_uncertainty_off: # reset the alignment probabilities to 1
			h_isize_prob[h_isize_prob] = 1.0
			c_isize_prob[c_isize_prob] = 1.0
			h_splits_prob[h_splits_prob] = 1.0
			c_splits_prob[c_splits_prob] = 1.0

		if options.internal_segment_based_only: # overlapping alignments are turned off 
			h_splits, h_splits_prob, c_splits, c_splits_prob = np.array([]), np.array([]), np.array([]), np.array([]) 

		if options.split_read_alignments_only: # internal segment based evidences are discarded
			h_isize, h_isize_prob, c_isize, c_isize_prob = np.array([]), np.array([]), np.array([]), np.array([]) 
		...
		#print(sm_likelihood_maximizer.loglikelihood( 0.0,0.0, delta, h_isize, h_isize_prob, h_splits, h_splits_prob, c_isize, c_isize_prob, c_splits, c_splits_prob))
		print('')		
		print(sm_likelihood_maximizer.returnMLECancerVAF(0.0, delta, h_isize, h_isize_prob, h_splits, h_splits_prob, c_isize, c_isize_prob, c_splits, c_splits_prob))
		print(sm_likelihood_maximizer.returnMLECancerVAF(0.5, delta, h_isize, h_isize_prob, h_splits, h_splits_prob, c_isize, c_isize_prob, c_splits, c_splits_prob))
		print(sm_likelihood_maximizer.returnMLECancerVAF(1.0, delta, h_isize, h_isize_prob, h_splits, h_splits_prob, c_isize, c_isize_prob, c_splits, c_splits_prob))
