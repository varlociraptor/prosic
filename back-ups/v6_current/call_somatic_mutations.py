...
def main():
	...
	# TODO determine mu and sigma for both the BAM files and pass them on to the LikelihoodMaximizer
 	...
 	sm_likelihood_maximizer 	= SMLikelihoodMaximizer(mu_h, sigma_h, mu_c, sigma_c, alpha = options.alpha, epsilon0 = options.epsilon_a, epsilon1 = options.epsilon_p, cancer_vaf_range = None, tolerance=options.tolerance) # deals with the likelihood function # TODO change 
	...
	for vcf_record in vcf_reader:  
		...
		if options.internal_segment_based_only: # overlapping alignments are turned off 
			h_overlapping_alignments = [] 
			c_overlapping_alignments = []

		if options.split_read_alignments_only: # internal segment based evidences are discarded
			h_paired_end_alignments = [] 
			c_paired_end_alignments = [] 
