...
LENGTH_RANGES 		= [[None, None], [None,9], [10,29], [1,20], [21,50], [30,49], [50, None], [50,99], [100, None], [250, None]]
...
def determineRecall(tp, n_true): 
	"""Returns the recall in string-form"""
	if n_true == 0:
		return '-.----'
	return "{:.4f}".format(float(tp)/float(n_true))

def determinePrecision(tp, n_calls): 
	"""Returns the precision in string-form"""
	if n_calls == 0:
		return '-.----'
	return "{:.4f}".format(float(tp)/float(n_calls))

def determineF1Score(tp, n_true, n_calls): 
	"""Returns the F1 score in string-form"""
	if n_true == 0 or n_calls == 0: 
		return '-.----'
	recall 		= float(tp) / float(n_true)
	precision 	= float(tp) / float(n_calls)
	if recall + precision == 0: 
		return '-.----'
	f1_score 	= 2.0*(recall*precision)/(recall + precision)
	return "{:.4f}".format(f1_score)	
...
class TestBinary: 
	...
	def printResults(self):			
		print("\n\n", self.title, "\n")
		print("DELETIONS")
		print("length range\tcancer vaf range\tgamma\t# true som.\t# som. calls\trecall\tprecision\tF1-score")
		print("------------\t----------------\t-----\t-----------\t------------\t------\t---------\t--------")
		for l in range(len(LENGTH_RANGES)):
			for c in range(len(CANCER_VAF_RANGES)):
				for g in range(len(GAMMA_LEVELS)):
					...
					recall_string 		= determineRecall(self.d_tp[l][c][g], self.d_n[l][c])
					precision_string 	= determinePrecision(self.d_tp[l][c][g], self.d_call[l][c][g])
					f1_score_string		= determineF1Score(self.d_tp[l][c][g], self.d_n[l][c], self.d_call[l][c][g])
					print(recall_string, '\t', precision_string, '\t', f1_score_string)
				print("------------\t----------------\t-----\t-----------\t------------\t------\t---------\t--------")
		
		print("\nDELETIONS -- NO BOUNDS ON THE CANCER VAF")
		print("length range\tgamma\t# true som.\t# som. calls\trecall\tprecision\tF1-score")	
		print("------------\t-----\t-----------\t------------\t------\t---------\t--------")
		for l in range(len(LENGTH_RANGES)):
			for g in range(len(GAMMA_LEVELS)):
				print(intervalToStr(LENGTH_RANGES[l]), '\t', end = '')
				...
				recall_string 		= determineRecall(self.d_tp[l][0][g], self.d_n[l][0])
				precision_string 	= determinePrecision(self.d_tp[l][0][g], self.d_call[l][0][g])
				f1_score_string		= determineF1Score(self.d_tp[l][0][g], self.d_n[l][0], self.d_call[l][0][g])
				print(recall_string, '\t', precision_string, '\t', f1_score_string)	
			print("------------\t-----\t-----------\t------------\t------\t---------\t--------")
		...
		for l in range(len(LENGTH_RANGES)):
			for c in range(len(CANCER_VAF_RANGES)):
				for g in range(len(GAMMA_LEVELS)):
					...
					recall_string 		= determineRecall(self.i_tp[l][c][g], self.i_n[l][c])
					precision_string 	= determinePrecision(self.i_tp[l][c][g], self.i_call[l][c][g])
					f1_score_string		= determineF1Score(self.i_tp[l][c][g], self.i_n[l][c], self.i_call[l][c][g])
					print(recall_string, '\t', precision_string, '\t', f1_score_string)
				print("------------\t----------------\t-----\t-----------\t------------\t------\t---------\t--------")
		...
		for l in range(len(LENGTH_RANGES)):
			for g in range(len(GAMMA_LEVELS)):
				...
				recall_string 		= determineRecall(self.i_tp[l][0][g], self.i_n[l][0])
				precision_string 	= determinePrecision(self.i_tp[l][0][g], self.i_call[l][0][g])
				f1_score_string		= determineF1Score(self.i_tp[l][0][g], self.i_n[l][0], self.i_call[l][0][g])
				print(recall_string, '\t', precision_string, '\t', f1_score_string)	
			print("------------\t-----\t-----------\t------------\t------\t---------\t--------")
		print("")
...
def main():
	...
	parser.add_option("--low", action="store_true", dest="low_precision", default=False,
						help="Uses low precision when matching detected deletions with the deletions in the true VCF file, i.e., the centerpoints must be less or exactly 100 bp away and the length must not differ more than 100 bp. This option overwrites the options '-d' and '-l' (!).")
	...
	parser.add_option("-d", action="store", dest="distance_threshold", default=0, type=int,
				  		help="Distance between centerpoints of deletions threshold. The distance must be smaller than this in order for the deletions to be deemed similar (in addition, the lengths must be similar, see option '-l'). (Default = 0 bp)")
	parser.add_option("-g", action="store", dest="own_gamma", type=float, default=None,
						help="In case you want to add your own preferred gamma (might be estimated!)")
	parser.add_option("-l", action="store", dest="length_threshold", default=0, type=int,
				  		help="Lengths of deletions threshold. The length difference must be smaller than this in order for the deletions to be deemed similar (in addition, the placement of the centerpoints must be similar, see option '-d'). (Default = 0 bp)")
	...
	call_vcf_filename = os.path.abspath(args[0])
	...
	if options.own_gamma != None: 
		GAMMA_LEVELS.append(options.own_gamma) 
		GAMMA_LEVELS.sort(reverse=True)
	
	print('INPUT:\tfile with ground truth @', os.path.abspath(options.truth_summary_file), '\n\tcalls in VCF file @', call_vcf_filename, '\n')
	

	truth_file = open(options.truth_summary_file, 'r') # true indels 
	call_vcf_reader = vcf.Reader(open(call_vcf_filename)) # called indels
	...
	for autosome in list_of_autosomes: 
		...
		while (returnAutosome(c_vcf_record.CHROM) == autosome):
			...
			if (not is_deletion and not is_insertion): # not an indel
				c_vcf_record = call_vcf_reader.next() 
			elif (not 'MAP_HEALTHY_VAF' in c_vcf_record.INFO): 
				# indel is not called and will be ignored
				print(c_vcf_record.INFO)
				c_vcf_record = call_vcf_reader.next() 
			else:
